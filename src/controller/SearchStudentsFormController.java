...
public class SearchStudentsFormController {
    ...
    public void initialize(){
        ...
        lastCol.setCellValueFactory(param -> {
            ...
            ImageView imgTrash = new ImageView("/view/assets/Edit.png");
            ImageView imgSearch = new ImageView("/view/assets/Search.png");

            imgEdit.getStyleClass().add("action-icons");
            imgTrash.getStyleClass().add("action-icons");
            imgSearch.getStyleClass().add("action-icons");
            ...
            imgSearch.setOnMouseClicked(event -> viewStudent(param.getValue()));

            return new ReadOnlyObjectWrapper<>(new HBox(10, imgEdit,imgTrash, imgSearch));
        });
    ...
    private void updateStudent(StudentTM tm){
        try {
            ...
            secondaryScene.setUserData("update");
    ...
    private void viewStudent(StudentTM tm){
        try {
            Stage secondaryStage = new Stage();
            FXMLLoader loader = new FXMLLoader(this.getClass().getResource("/view/MainForm.fxml"));
            Scene secondaryScene = new Scene(loader.load());
            MainFormController ctrl = loader.getController();
            secondaryScene.setUserData("view");
            ...
            secondaryStage.initModality(Modality.WINDOW_MODAL);
            secondaryStage.initOwner(txtQuery.getScene().getWindow());
            secondaryStage.setTitle("View Student");
            ctrl.navigate("View Student","/view/StudentForm.fxml", AppBarIcon.NAV_ICON_NONE, null, tm);
            secondaryStage.showAndWait();
            tblResults.refresh();
        } catch (IOException e) {
            e.printStackTrace();
